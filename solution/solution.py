...
class LLMJudge:
    ...
    def detect_bias(self, scores_batch: list[dict[str, Any]]) -> dict[str, Any]:
        ...
        def avg_of(entry: dict[str, Any]) -> float:
            scores = entry.get("scores", {})
            if not scores:
                return 0.0
            return sum(scores.values()) / len(scores)

        entry_averages = [avg_of(entry) for entry in scores_batch]
        overall_mean = sum(entry_averages) / len(entry_averages)

        leniency_bias = overall_mean > 0.8
        severity_bias = overall_mean < 0.3

        first_position = entry_averages[0::2]
        second_position = entry_averages[1::2]
        positional_bias = False
...
class BenchmarkRunner:
    ...
    def generate_report(self, results: list[EvalResult]) -> dict[str, Any]:
        ...
        avg_context_recall = avg(recalls) if recalls else None
        avg_context_precision = avg(precisions) if precisions else None
    ...
    def run_regression(self, new_results: list, baseline_results: list) -> dict:
        ...
        new_avg_faithfulness = avg([r.faithfulness for r in new_results])
        new_avg_relevance = avg([r.relevance for r in new_results])
        new_avg_completeness = avg([r.completeness for r in new_results])

        baseline_avg_faithfulness = avg([r.faithfulness for r in baseline_results])
        baseline_avg_relevance = avg([r.relevance for r in baseline_results])
        baseline_avg_completeness = avg([r.completeness for r in baseline_results])
...
class FailureAnalyzer:
    ...
    def generate_improvement_log(self, failures: list, suggestions: list[str]) -> str:
        ...
        for i, failure in enumerate(failures):
            ...
            fix = suggestions[i] if i < len(suggestions) else (suggestions[-1] if suggestions else "")
            lines.append(f"| {failure_id} | {failure.failure_type} | {root_cause} | {fix} | Open |")
        return "\n".join(lines)
    ...
    def generate_improvement_suggestions(
        ...
        if categories.get("hallucination"):
            suggestions.append(
                "Implement a hallucination/grounding checker to filter claims unsupported by context"
            )
        ...
        if categories.get("incomplete"):
            suggestions.append(
                "Increase chunk size or top-k retrieval to reduce context fragmentation"
            )
        if categories.get("off_topic"):
            suggestions.append(
                "Review intent detection so the agent routes to the correct topic"
            )

        fallback_suggestions = [
            "Add few-shot examples showing complete, well-grounded answers",
            "Expand the golden dataset with more edge cases to catch regressions early",
            "Set up periodic human calibration of the LLM judge",
        ]
